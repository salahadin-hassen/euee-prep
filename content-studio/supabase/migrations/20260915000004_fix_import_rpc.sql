-- Idempotent fix: ensure import_manual_questions exists in the database.
-- Safe to apply regardless of whether prior migrations ran.

-- 1. manual_imports table (idempotent)
...
begin
  if not exists (
    select 1 from pg_policies where policyname = 'admins can read manual imports'
      and tablename = 'manual_imports' and schemaname = 'public'
  ) then
...
-- 2. manual_import_id column on questions (idempotent)
...
-- 3. Drop and recreate the function with source_pdf_page support
