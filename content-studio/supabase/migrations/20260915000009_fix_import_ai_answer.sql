-- Fix: store AI predicted answer from correct_choice_index during import.
...
begin
  ...
  for question in
    select jsonb_array_elements(
      case
        when jsonb_typeof(p_raw_payload) = 'array' then p_raw_payload
        when p_raw_payload ? 'questions' then p_raw_payload->'questions'
        when p_raw_payload ? 'chapters' then coalesce(
          (
  ...
  loop
    ...
    where c #>> '{}' is not null and nullif(btrim(c #>> '{}'), '') is not null;

    while jsonb_array_length(q_choices) < 4 loop
      q_choices := q_choices || '""'::jsonb;
    end loop;

    if jsonb_array_length(q_choices) > 4 then
      q_choices := (select jsonb_agg(val) from jsonb_array_elements(q_choices) with ordinality as t(val, idx) where idx <= 4);
    end if;

    q_explanation := coalesce(question->>'explanation', '');
    q_source_page := nullif(question->>'source_page', '')::integer;
    q_correct_index := nullif(question->>'correct_choice_index', '')::integer;

    insert into public.questions (
      project_id, order_index, question_text, choices,
      correct_answer, explanation_draft, status, manual_import_id,
      source_pdf_page, ai_predicted_choice_index
    ) values (
      p_project_id, next_order, q_prompt, q_choices,
      '', q_explanation, 'unverified', import_id,
      q_source_page, q_correct_index
    );
